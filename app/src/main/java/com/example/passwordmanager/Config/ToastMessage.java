package com.example.passwordmanager.Config;

public class ToastMessage {
    public static final String PRESS_DOUBLE_TO_EXIT = "Please click BACK again to exit";
    public static final String COPIED_TO_CLIPBOARD = "Copied to clipboard!";
    public static final String CANT_USE_BIOMETRICS = "Can't user your biometrics right now!";


    public static final String SETUP_LOGIN_TYPE_ERROR = "Can't set your login type!";
    public static final String GET_LOGIN_TYPE_ERROR = "Can't get your login type!";
    public static final String REMOVE_LOGIN_TYPE_ERROR = "Can't remove current login type!";



    public static final String REMOVE_LOGIN_PASSWORD_ERROR = "Can't remove current login password!";
    public static final String VERIFY_LOGIN_PASSWORD_ERROR = "Can't verify your login password!";
    public static final String GET_LOGIN_PASSWORD_ERROR = "Can't get your login password!";


    public static final String CANT_FORMAT_IMAGE = "The image that you selected can't be formatted!";
    public static final String CORRUPTED_PRE_INSTALLED_ICON = "The selected icon might be corrupted!";
    public static final String CANT_GENERATE_ICON = "Can't generate icon!";


    public static final String CORRUPTED_TABLE_PASSWORDS = "Passwords table might be corrupted!";
    public static final String CANT_GET_PASSWORD = "Some of your passwords might be corrupted!";
    public static final String CANT_REMOVE_PASSWORD = "Can't remove your password!";
    public static final String CANT_REMOVE_ALL_PASSWORDS = "Can't remove your passwords!";
    public static final String CANT_UPDATE_PASSWORD = "Can't update your password!";
    public static final String CANT_ADD_PASSWORD = "Something went wrong with adding your password!";
    public static final String CANT_LOAD_PASSWORD = "Something went wrong with loading your password!";

    public static final String CANT_DOWNLOAD_DATA = "Something went wrong with saving your passwords!";
}
